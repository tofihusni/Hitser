...
const DEFAULTS = {
  ...
  allowSteal: true,
  placeSeconds: 60,
  stealSeconds: 35, // hay que escribir título y artista, y elegir el hueco
...
class Game {
  ...
  addPlayer(id, name, avatar) {
    ...
    const p = {
      ...
      tokens: 0,
      connected: true,
      stats: { correct: 0, wrong: 0, steals: 0, tokensEarned: 0, streak: 0, bestStreak: 0 },
    };
    this.players.push(p);
    return p;
  }
  ...
  start() {
    ...
    for (const p of this.players) {
      ...
      p.stats = { correct: 0, wrong: 0, steals: 0, tokensEarned: 0, streak: 0, bestStreak: 0 };
    }
  ...
  revealSimul() {
    ...
    for (const r of results) {
      ...
      // Racha: cada 3 aciertos seguidos, ficha extra.
      r.streakBonus = r.correct && p.stats.streak > 0 && p.stats.streak % 3 === 0;
      if (r.streakBonus) {
        p.tokens += 1;
        p.stats.tokensEarned += 1;
      }
      r.streak = p.stats.streak;
  ...
  // Otro jugador paga 1 ficha y apuesta a que el activo falló. Para robar hay
  // que hacer dos cosas bien: elegir el hueco correcto en SU propia línea de
  // tiempo Y acertar el título y el artista de la canción.
  stealBid(playerId, gap, guess) {
    ...
    if (p.tokens < 1) return { error: 'No tienes fichas' };
    const artist = String((guess && guess.artist) || '').trim();
    const title = String((guess && guess.title) || '').trim();
    if (!artist || !title) return { error: 'Para robar, escribe el título y el artista' };
    gap = Number(gap);
    if (!Number.isInteger(gap) || gap < 0 || gap > p.timeline.length) {
      return { error: 'Posición no válida' };
    }
    p.tokens -= 1;
    this.stealBids.push({ playerId, gap, guess: { artist, title } });
    return { ok: true };
  }
  ...
  reveal() {
    ...
    if (correct) {
      ...
      p.stats.streak += 1;
      p.stats.bestStreak = Math.max(p.stats.bestStreak, p.stats.streak);
      streakBonus = p.stats.streak % 3 === 0;
      if (streakBonus) {
        p.tokens += 1;
        p.stats.tokensEarned += 1;
      }
    ...
    // Los robos se evalúan siempre (para poder explicar qué falló). Solo se
    // llevan la carta si el activo falló, el hueco es correcto en SU línea y
    // además aciertan título y artista.
    for (const bid of this.stealBids) {
      const q = this.player(bid.playerId);
      if (!q) continue;
      bid.gapOk = isCorrectGap(q.timeline, bid.gap, card.year, margin);
      bid.artistOk = fuzzyMatch(bid.guess ? bid.guess.artist : '', card.artist);
      bid.titleOk = fuzzyMatch(bid.guess ? bid.guess.title : '', card.title);
      bid.songOk = bid.artistOk && bid.titleOk;
      bid.won = !correct && bid.gapOk && bid.songOk && stealWinnerId === null;
      if (bid.won) {
    ...
    this.lastResult = {
      ...
      stealBids: this.stealBids.map((b) => ({
        playerId: b.playerId,
        gapOk: !!b.gapOk,
        artistOk: !!b.artistOk,
        titleOk: !!b.titleOk,
