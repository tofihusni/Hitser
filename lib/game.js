...
function isCorrectGap(timeline, gap, year) {
...
class Game {
  ...
  addPlayer(id, name) {
    ...
    const p = { id, name, timeline: [], tokens: 0, connected: true };
    this.players.push(p);
    return p;
  }
  ...
  start() {
    ...
    this.deck = shuffle(this.songs.map((_, i) => i), this.rng);
  ...
  drawCard() {
    if (this.deck.length === 0) {
      ...
      const rest = this.songs.map((_, i) => i).filter((i) => !used.has(i));
      this.deck = shuffle(rest.length ? rest : this.songs.map((_, i) => i), this.rng);
    }
  ...
  reveal() {
    ...
    const correct = isCorrectGap(p.timeline, this.placedGap, card.year);
    ...
    if (this.guess && !this.clueShown) {
      ...
      if (guessResult.tokenWon) p.tokens += 1;
    }

    let stealWinnerId = null;
    if (correct) {
      p.timeline.splice(this.placedGap, 0, card);
    } else {
      ...
      for (const bid of this.stealBids) {
        ...
        if (q && isCorrectGap(q.timeline, bid.gap, card.year)) {
          q.timeline.splice(bid.gap, 0, card);
          stealWinnerId = q.id;
          break;
        }
      }
  ...
  viewFor(playerId) {
    ...
    return {
      ...
      turnPlayerId: this.activePlayer ? this.activePlayer.id : null,
...
module.exports = { Game, DEFAULTS, shuffle, normalize, fuzzyMatch, isCorrectGap, correctGaps, insertCard, levenshtein };
